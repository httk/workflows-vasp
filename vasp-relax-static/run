...
job inputs and parameters are documented in :mod:`httk.workflow.vasp.runners`. Nothing here imports
...
def promote(a: Attempt) -> None:
    ...
    for name in names_parameter(a, "collect", DEFAULT_COLLECT):
    ...
    options = replace(
        ...
        incar_tags={
            **(tags_parameter(a, "static_incar_tags") or DEFAULT_STATIC_TAGS),
            **tags_parameter(a, "incar_tags"),
        },
    )
...
def publish(a: Attempt) -> None:
    ...
    prefix = text_parameter(a, "data_prefix", "")
